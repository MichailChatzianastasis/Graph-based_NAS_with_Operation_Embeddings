...
parser.add_argument('--saved_model', action='store_true' , default=False,
                    help='load saved pth model,optimizer,scheduler file')
...
# otherwise process the raw data and save to .pkl
    ...
    if args.data_type == 'ENAS':
        ...
        train_data , test_data ,graph_args = load_NASBENCH_graphs(args.data_name)
...
'''Prepare the model'''
# model
if (args.model.startswith("DVAE_EMB") or args.model.startswith("SVAE_EMB") or args.model.startswith("DVAE_GCN_EMB")):
...
#load model if args saved-model
if args.saved_model:
    model.load_state_dict(torch.load(os.path.join(args.res_dir,'model_checkpoint300.pth'),map_location='cuda:0'))
...
if args.saved_model:
    optimizer.load_state_dict(torch.load(os.path.join(args.res_dir,'optimizer_checkpoint300.pth'),map_location='cuda:0'))
#load optimizer

scheduler = ReduceLROnPlateau(optimizer, 'min', factor=0.1, patience=10, verbose=True)
#load scheduler
if args.saved_model:
    scheduler.load_state_dict(torch.load(os.path.join(args.res_dir,'scheduler_checkpoint300.pth'),map_location='cuda:0'))
...
if (args.model.startswith("DVAE_EMB")):
    print("embeddings", model.embeddings.weight)
...
if args.only_test:
    save_latent_representations(1)
    exit()
    if args.predictor:
        Nll, acc, pred_rmse = test()
    else:
        if (args.model == "DVAE_EMB"):
            print("embeddings", model.embeddings.weight)
        Nll, acc = test()
        pred_rmse = 0
    epoch = args.continue_from if args.continue_from is not None else 0
