...
'''
SVAE-EMB Not implemented yet. 
'''

class SVAE_EMB(nn.Module):
    def __init__(self, max_n, nvt, emb_size, START_TYPE, END_TYPE, hs=501, nz=56, bidirectional=False):
        super(SVAE_EMB, self).__init__()
        self.max_n = max_n  # maximum number of vertices
        self.nvt = nvt  # number of vertex types
        self.emb_size = emb_size # size of operation embeddings
        self.xs = (emb_size + max_n-1)  # size of input x for GRU,
                                   # [emb_size, bit(connections)]
        self.START_TYPE = START_TYPE
        self.END_TYPE = END_TYPE
        self.hs = hs  # hidden state size of each vertex
        self.nz = nz  # size of latent representation z
        ...
        # 0. encoding-related
        #embeddings-related
        self.embeddings = nn.Embedding(8,emb_size)
    ...
    def _collate_fn(self, G):
        # create mini_batch of tensors from list G by padding
        # each graph g is a 1 * (n_vertex - 1) * (emb_size + n_vertex-1) tensor
        # pad all to 1 * (max_n - 1) * (emb_size + max_n-1) tensors
        if type(G) != list:
            G = [G]
        G_new = []
        for g in G:
            if g.shape[1] < self.max_n - 1:
                ...
                padding[0, :, self.START_TYPE] = 1  # use start type's bit to indicate padding 
                                                    # nodes (since start types are never predicted)
                g = torch.cat([g, padding], 1)
            if g.shape[2] < self.nvt + self.max_n -1:
                padding = torch.zeros(1, g.shape[1], self.nvt + self.max_n -1 -g.shape[2]).to(self.get_device())
                g = torch.cat([g, padding], 2)  # pad zeros to indicate no connections to padding 
                                                # nodes
            G_new.append(g)
        return torch.cat(G_new, 0)
    ...
    def encode(self, G):
        ...
        # we must transform G to : [batch_size * max_n-1 * (xs=emb_size+max_n-1)]
        G_new = []
        #print(len(G),len(G[0]),"G")
        for g in G:
            #print("g",g.shape)
            one_hot = g[:,:self.nvt]
            #print(one_hot.shape,"one_hot")
            indexes = torch.argmax(one_hot, axis=1) 
            #print(indexes.shape,"indexes")
            #   indexes = torch.tensor([[i] for i in indexes])
            embeddings = self.embeddings(indexes.to(self.get_device()))
            #print(embeddings.shape,"embeddings")
            g = torch.cat((embeddings,g[:,self.nvt:]),axis=1)
            #print(g.shape,"g_Final")
            G_new.append(g)
        G=torch.stack(G_new)
        _, Hn = self.grue(G)
        Hg = Hn.view(Hn.shape[1], -1)   # Hn's second dimension is "batch"
...
class DVAE(nn.Module):
    def __init__(self, max_n, nvt, START_TYPE, END_TYPE, hs=501, nz=56, bidirectional=False, vid=True):
        ...
        self.max_n = max_n  # maximum number of vertices
        print("maximum nubmer of vertices",max_n)
        self.nvt = nvt  # number of vertex types
        self.START_TYPE = START_TYPE
        self.END_TYPE = END_TYPE
        self.hs = hs  # hidden state size of each vertex
...
'''
D-VAE_EMB: DVAE with operation embeddings instead of one-hot vectors.
'''


class DVAE_EMB(nn.Module):
    def __init__(self, max_n, nvt, emb_size, START_TYPE, END_TYPE, hs=501, nz=56, bidirectional=False, vid=True):
        super(DVAE_EMB, self).__init__()
        pretrained_weight= np.array([[-1.505, -0.665, -0.641],[-1.570, -0.080, -0.590],[-0.222, -1.357,  0.844],
                                         [ 0.775,  1.90, -0.552], [ 0.253, -0.425, -0.549],[-0.091, -0.657,  0.355]
                                        ,[ 0.319,  1.108,  -0.085],[-0.335, -1.564,  2.217]]
                                        ) #this is from DVAE_EMB5 results
        self.max_n = max_n  # maximum number of vertices
        print("maximum nubmer of vertices", max_n)
        self.nvt = nvt  # number of vertex types
        self.emb_size = emb_size # size of operation embeddings
        ...
        # embeddings-related
        self.embeddings =  nn.Embedding(8,emb_size)   # number of different operations , size of each embedding
        #use pretrained embeddings
        self.embeddings.weight.data.copy_(torch.from_numpy(pretrained_weight))
        # 0. encoding-related
        self.grue_forward = nn.GRUCell(emb_size, hs)  # encoder GRU
        self.grue_backward = nn.GRUCell(emb_size, hs)  # backward encoder GRU
        self.fc1 = nn.Linear(self.gs, nz)  # latent mean
        self.fc2 = nn.Linear(self.gs, nz)  # latent logvar

        # 1. decoding-related
        self.grud = nn.GRUCell(emb_size, hs)  # decoder GRU
        self.fc3 = nn.Linear(nz, hs)  # from latent z to initial hidden state h0
        self.add_vertex = nn.Sequential(
            nn.Linear(hs, hs * 2),
            nn.ReLU(),
    ...
    def _nodes_index(self, idx):
        if type(idx) in [list, range]:
            ...
            idx = torch.LongTensor([idx]).unsqueeze(0)
        return idx.float().to(self.get_device())
    ...
    def _get_embeddings(self,X):
        ...
        X_emb=[]
        for label in X:
            X_emb.append(self.embeddings[label.data.tolist()[0]])
        X_emb=torch.tensor(X_emb).float().to(self.get_device())
        return X_emb
    ...
    def _propagate_to(self, G, v, propagator, H=None, reverse=False):
        ...
        if H is not None:
            ...
            H = H[idx]
        v_types = [g.vs[v]['type'] for g in G]
        idx = self._nodes_index(v_types) # get indexes
        if(len(G)>1):
            X = torch.squeeze(self.embeddings(idx.type(torch.LongTensor).to(self.get_device()))) # get embeddings from one hot vector
        else:
            X=torch.squeeze(self.embeddings(idx.type(torch.LongTensor).to(self.get_device())),1)
        if reverse:
            H_name = 'H_backward'  # name of the hidden states attribute
            H_pred = [[g.vs[x][H_name] for x in g.successors(v)] for g in G]
            if self.vid:
        ...
        if H is None:
            ...
            if max_n_pred == 0:
                ...
                H_pred = [torch.cat(h_pred +
                          ...
                          for h_pred in H_pred]  # pad all to same length
                H_pred = torch.cat(H_pred, 0)  # batch * max_n_pred * vs
                H = self._gated(H_pred, gate, mapper).sum(1)  # batch * hs    this is the Hin: The concatenated message from the neighbours.
        Hv = propagator(X,H) # we pass the H:the message from the neighbors, and the X:the embedding of the node, to the GRU.
        #Hv is the new hidden state of the vertex v
        for i, g in enumerate(G):
           g.vs[v][H_name] = Hv[i:i + 1]
        return Hv
...
'''
D-VAE_EMB_ATT: DVAE_EMB with attention mechanism. Not implemented yet.
'''


class DVAE_EMB_ATT(nn.Module):
    def __init__(self, max_n, nvt, emb_size, START_TYPE, END_TYPE, hs=501, nz=56, bidirectional=False, vid=True):
        super(DVAE_EMB_ATT, self).__init__()
        pretrained_weight= np.array([[-1.505, -0.665, -0.641],[-1.570, -0.080, -0.590],[-0.222, -1.357,  0.844],
                                         [ 0.775,  1.90, -0.552], [ 0.253, -0.425, -0.549],[-0.091, -0.657,  0.355]
                                        ,[ 0.319,  1.108,  -0.085],[-0.335, -1.564,  2.217]]
                                        ) #this is from DVAE_EMB5 results
        ...
        #attention-related https://docs.dgl.ai/en/0.4.x/tutorials/models/1_gnn/9_gat.html
        self.fch = nn.Linear(self.vs, self.hs, bias=False) #eq 1
        self.attn_fc = nn.Linear(self.hs, 1, bias=False) #eq 2
    ...
    def _propagate_to(self, G, v, propagator, H=None, reverse=False):
        ...
        if H is None:
            ...
            if max_n_pred == 0:
                ...
                H = self._gated(H_pred, gate, mapper).sum(1)  # batch * hs    this is the Hin: The concatenated message from the neighbours.
                print("H line 1446 :", "H length", len(H), "H[0] length", len(H[0]),
                      "H data", H)
...
'''
    D-VAE with GCN encoder with operation embeddings
    The message passing happens at all nodes simultaneously.
'''
class DVAE_GCN_EMB(DVAE_EMB):
    def __init__(self, max_n, nvt, emb_size, START_TYPE, END_TYPE, hs=501, nz=56, bidirectional=False, levels=3):
        # bidirectional means passing messages ignoring edge directions
        super(DVAE_GCN_EMB, self).__init__(max_n, nvt, emb_size, START_TYPE, END_TYPE, hs, nz, bidirectional)
        self.levels = levels
        self.gconv = nn.ModuleList()
        self.gconv.append(
                nn.Sequential(
                    nn.Linear(emb_size, hs), 
                    nn.ReLU(), 
                    )
                )
        for lv in range(1, levels):
    ...
    def _get_zero_x(self, n=1):
        # get zero predecessor states X, used for padding
        return torch.zeros(n, self.emb_size).to(self.get_device())
...
'''
    A fast D-VAE variant with Embeddings.
    Use D-VAE's encoder + S-VAE's decoder to accelerate decoding.
'''
class DVAE_EMB_fast(DVAE_EMB):
    def __init__(self, max_n, nvt, emb_size,START_TYPE, END_TYPE, hs=501, nz=56, bidirectional=False):
        super(DVAE_EMB_fast, self).__init__(max_n, nvt,emb_size, START_TYPE, END_TYPE, hs, nz, bidirectional)
        self.grud = nn.GRU(hs, hs, batch_first=True)  # decoder GRU
        self.add_vertex = nn.Sequential(
                nn.Linear(self.hs, self.hs),
                nn.ReLU(),
